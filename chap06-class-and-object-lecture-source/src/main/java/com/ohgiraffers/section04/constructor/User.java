...
public class User {
     ...
     * 2. 매개변수가 있는 생성자의 경우 매개변수로 전달받은 값으로 필드를 초기화하며,
     *    인스턴스를 생성할 목적으로 주로 사용한다.
     * 3. 작성한 생성자 외에는 인스턴스를 생성하는 방법을 제공하지 않겠다는 의미(초기값 전달 강제)
     */

    /*
     * 생성자 작성 시 주의할 점
     * 1. 생성자 메소드는 반드시 클래스의 이름과 동일해야한다. (대/소문자까지)
     * 2. 생성자 메소드는 반환형을 작성하지 않는다. (반환형 작성시 일반 메소드로 인식됨)
     */
     ...
     * 2. 매개변수 있는 생성자
     *
     * 매개변수가 있는 생성자가 하나라도 존재하면 기본생성자를 java compiler가 자동으로 추가하지 않는다.
     * -> 이 경우에 기본 생성자가 필요한 경우에는 반드시 명시적으로 작성을 해야한다.
     */
    ...
    public String getInfo() {
        return "'" + this.name + "'의 정보 [id : " + this.id + " / pwd : " + this.pwd + " / enrollDate : " + this.enrollDate + "]";
    }
    ...
    // 모든 필드를 초기화하는 생성자 (id, pwd, name, enrolledDate)
...
//        this.enrollDate = enrollDate;
         ...
         * this()는 가장 첫 줄에 선언해야하며, 그렇지 않은 경우 compile error가 발생한다.
         */
